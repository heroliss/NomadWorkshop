...
> 状态：**研究基线 + 首个生产长跑 v0.2**，更新于 2026-09-05。本文记录可复用机制、采用边界和来源，避免后续只凭记忆重复搜集。项目仍应在真实任务中验证并修改这些判断；这里不是第三方项目排行榜，也不是安装清单。
...
《游牧工坊》现已实现第一条 L0 长跑：`FoundationSoak` 只在暂停且建造事务空闲时，通过 Command 按固定整数毫秒复用生产模拟步；有明确的步数 / 步长 / 停滞预算，并输出 Seed、版本、停止原因、状态极值、行动增量、水量偏差、故障 Tick 与轨迹校验和。同 Seed / 同设置可逐步重放，多 Seed 则用于暴露统计形状；同一安全业务检查点经完整恢复后的后续轨迹也可精确重放。首轮运行发现“归一化爱好偏好被直接当作 Utility，画架导致散步候选消失”，修复后才增加分布回归；这正是 Harness 应先发现问题、再形成门禁，而不是只证明既有实现正确的例子。
...
4. **第一个长跑（已完成最小版）**：固定 Seed、时限、循环 / 无进展检测、步数预算和同一安全检查点的确定继续已进入 Foundation；继续量化相对未中断轨迹的业务差异、真实文件往返、故障时间分布和证据导出，仍由当前任务拥有生命周期，不启动无限自治服务。
