...
同一个 Agent 更换长对话时也使用这份最小包。先完成当前小阶段的验证与场景保存，核对未结束 job 和脏文件，再开启新任务；新任务首次仍核验实际现场，不能把交接中的“已停止/已提交”当实时事实。自动压缩本身不要求换任务，同一个未解决问题通常继续原任务。

游牧工坊可用以下简短提示承接，无需粘贴完整聊天：

```text
继续 D:/SSFramework 的《游牧工坊》首版可玩美术样板目标。
先读适用的 AGENTS.md、docs/ai-project-index.md 与它指向的 Foundation §9，
按当前队列和专题证据核验 Git、Unity 与未完成操作，再推进下一个小阶段。
保持 J 参考的温暖工业风、可替换人物/设施和真实完整旅程；同步优化有证据支持的
AI 工作流与 SSFramework 边界。功能与验证收口后可以阶段性本地提交，不推送。
使用目标模式持续推进，工程通过与用户美术/体验验收分别记录；有重大方向疑问再讨论。
```
