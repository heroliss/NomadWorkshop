...
多人、F2P / IAP、UGC / Mod、XR、主机认证、运行时 LLM NPC 和长期 LiveOps 仍是条件分支，不为图谱完整预建。《游牧工坊》当前确实需要**种子宏观地图、固定旅途舞台、有限停靠场景和分级持久化**，因此只把这条受控能力进入 `Now`；它不等于预先建设可复用于所有游戏的一比一开放世界框架。
...
| G2 | 系统、AI、成长、经济与平衡 | M4 → M4 | 固定 Seed、双后端对拍、黄金轨迹；Foundation 首个有界多 Seed 长跑已能审计状态极值、行动分布、停滞与水量守恒，并实际抓出爱好偏好量纲问题；同一安全检查点可确定继续 | 量化相对未中断轨迹的业务差异并补真实故障时间分布；自动结果仍需真实玩家难度与节奏校准 |
...
| V1 | Art Direction、2D / 3D 资产 | M2 → M3 | `imagegen` 可做概念与 2D 原型；[Blender 资产管线](blender-art-pipeline.md)已用 5.2.1 LTS 完成脚本化 `.blend` / FBX / 预览 / manifest，并把首个静态道具闭环到 Unity；[3D 资产族策略](nomad-workshop-asset-family-pipeline.md)已定义 Art Bible、PBR、状态、动画、音效与 Provider 的接缝 | 用代表性 Style Kit 冻结首版 Art Bible；完成外部 AI Mesh 盲测、第二种资产复验、同族成组镜头、合批 / 性能、法线和正式授权台账 |
| V2 | Shader、VFX、材质、灯光、镜头与动画 | M3 → M4 | Unity 6 + URP 17.3；默认 3D / 显式 2D 迁移、内部 HDR、SSAO、两级软阴影、ACES、程序化天空与首帧局部反射已进入正式 Foundation 并有实际 Game View 证据；资产族策略已分离 Base PBR、环境覆盖、Decal、Overlay / VFX 与 Mesh 阈值 | 在车辆代表性镜头做首个可组合 `SurfaceStateful` Spike，并对共享 Humanoid 完成 Rig / Retarget / Foot-Hand 接触往返；拆分 Desktop / Mobile 质量资产，以 RenderGraph 和目标设备证据收敛性能 |
...
| Q3 | Agent Playtest 与真实玩家测试 | M2 → M4 | 业务 Command 玩家路径 smoke；[Harness 调研](ai-game-development-harness-research.md)已形成 Trace、双通道观察、Evidence Bundle 与采用边界；Foundation 有首个固定步长、确定重放、有界停止的 L0 长跑 | 加中途存取轨迹、极薄 Input Driver 和可回放动作轨迹；另建真实玩家观察和访谈循环 |
...
| 3D 制作自动化 | [Blender 5.2 CLI / Python 基线](blender-art-pipeline.md)；[Blender MCP](https://github.com/ahujasid/blender-mcp)仅为交互候选 | 第二种几何和 Mesh 合并 / 性能复验；只有交互修改成为真实瓶颈才隔离评估 MCP | 首个静态道具的 Blender → Unity Importer → URP 材质 → Prefab / Collider → 次级 Universal Renderer → 测试 / 人工 Game View 闭环已验证 |
| AI 3D 原型 | [3D 资产族策略](nomad-workshop-asset-family-pipeline.md)中的 Rodin / Meshy / Tripo 云端挑战者与条件式本地模型 | 用尚无 Mesh 的新资产检验 Brief → Hero 概念 → 按需参考 → 3D → 清理 → Unity；多图只为关键接口增加，达到真实批量后才接 API / Skill | 首轮先完成车载厨房 Hero 概念，尚未上传外部 Provider 或消耗额度 |
...
- 以已通过的 Blender → Unity → 次级 Universal Renderer 静态道具闭环为起点，用第二种几何验证 Mesh 合并、运行时顶点、Draw Call、资产 brief、来源和车辆代表性镜头；第二次稳定复现后再考虑 `blender-asset-pipeline` Project Skill。
...
- `docs/blender-art-pipeline.md`：Blender 5.2 CLI / Python 基线、插件边界和首个静态道具的 Unity Import 闭环。
