...
> 状态：**统一模拟 Tick / 生活日与确定性沙尘天气 + 居民脚底根节点 + mL 水循环 / 饮水站实例库存 + 正向娱乐 / 心情 / 疲劳 / 压力连续状态 + 可建造观景画架 / 真实爱好恢复 + 平滑需求压力 / 确定性随机 + 有界长时模拟审计 + 设施状态 / 实体备件维修 + 共享吸附基格 / 跨设备镜头 + 上下文行动规划 + 实体水罐 / 物品放置区域 / 台面杯具原子拿放 + 旱厕 + 精确功能点停靠 / 原子资源改道 + Framework 分层连续建造 / 可回滚 NavMesh + Foundation 运行检查点 + 默认 URP 3D / 显式 2D 兼容 / HDR-PBR 图形基线 + 自主休整 + 互斥式居民 HUD + 导航 / 交互 Harness + 3D 资产 Harness v0.39**，更新于 2026-09-05。当前已经跑通“玩家连续摆放设施并预演新旧功能点与物品停放区 → 几何合法即可更新导航并提交 → 杯具按台面区域与精确姿态落位、联合锁定来源/目标、由居民拿起携带并原子放下 → 居民自动避让施工占地 → 完整水罐方案经 Utility 决策 → 装水 / 搬运 / 精确落位 / 饮用 / 代谢 / 平滑产生如厕意图 → 世界 Seed 与统一 Tick 重建同一场沙尘暴 → 风暴连续加速水箱积尘、欠保养、磨损与风险 → 出水阀故障安全中断未取货搬运 → 居民从维护托盘拿取唯一实体维修包、精确前往维修阀、投入工时并原子消耗备件 → 修复后继续守恒水循环 → 空闲时漫步、发呆或前往观景画架作画 → 暂停态跨 Seed / 步长审计轨迹、停滞、守恒与检查点差异预算 → 施工切路后按设施功能组恢复或改道 → 已提交世界捕获 / 原子落盘 / 加载重建”的可恢复闭环；它还不是完整 Foundation Prototype、商业垂直切片、玩家存档交互或正式美术基线。产品真值见 [`docs/nomad-workshop-game-vision.md`](../../../docs/nomad-workshop-game-vision.md)。
...
- 设施状态目前只有车辆水箱拥有非零暴露配置，也只有“出水阀卡滞”一个具体故障；正式环境只有每生活日一次的简单确定性沙尘窗口，维修链只有初始水箱维护托盘中的一只维修包。尚未接入区域 / 旅途天气、天气预报与遮蔽、备件补货或制造、维修工具 / 专用动画、其他设施故障谱、多日故障分布或长时间参数平衡；
...
下一步先用多 Seed / 多生活日统计真实沙尘下的故障与维修分布，并把检查点对照穿过真实 `IStorageUtility` 文件往返；同时补出“维修包耗尽后的可解释阻塞 → 补货或制造”最小供应链，避免当前一次性初始备件让长局失去恢复路径。随后再在杯具上补最薄 Humanoid/IK Adapter 与真实上下文动机；一份餐食、蓝图搬料施工、玩家存档 UI 和正式多居民 Agent 继续按风险后移，不为扩内容复制一套任务执行器。
