...
> 状态：**统一模拟 Tick / 生活日投影 + 居民脚底根节点 + mL 水循环 / 饮水站实例库存 + 正向娱乐 / 心情 / 疲劳 / 压力连续状态 + 可建造观景画架 / 真实爱好恢复 + 平滑需求压力 / 确定性随机 + 设施状态 / 车辆水箱故障闭环 + 共享吸附基格 / 跨设备镜头 + 上下文行动规划 + 实体水罐 / 物品放置区域 / 台面杯具 + 旱厕 + 精确停靠 / 原子资源改道 + Framework 分层连续建造 / 可回滚 NavMesh + Foundation 运行检查点 + 默认 URP 3D / 显式 2D 兼容 / HDR-PBR 图形基线 + 自主休整 + 互斥式居民 HUD + 导航 / 交互 Harness + 3D 资产 Harness v0.36**，更新于 2026-09-05。当前已经跑通“玩家连续摆放设施并预演新旧功能点与物品停放区 → 几何合法即可更新导航并提交 → 杯具按台面区域与精确姿态落位并完成存档往返 → 居民自动避让施工占地 → 完整水罐方案经 Utility 决策 → 装水 / 搬运 / 精确落位 / 饮用 / 代谢 / 平滑产生如厕意图 → 车辆水箱积尘、欠保养与磨损连续积累 → 出水阀故障安全中断未取货搬运 → 修复后继续守恒水循环 → 空闲时漫步、发呆或前往观景画架作画 → 施工切路后按设施功能恢复或改道 → 已提交世界捕获 / 原子落盘 / 加载重建”的可恢复闭环；它还不是完整 Foundation Prototype、商业垂直切片、玩家存档交互或正式美术基线。产品真值见 [`docs/nomad-workshop-game-vision.md`](../../../docs/nomad-workshop-game-vision.md)。
...
- `PlacementRegionLedger` 已把设施局部放置区域、物品 footprint、稳定候选姿态与预留事务纳入无 Unity 依赖的空间真值；`NomadWorldItemDefinition` 统一声明类别、尺寸、安全边距、朝向和灰盒表现。车辆水箱/饮水站声明水罐停放区，野战厨房声明台面区；唯一水罐与首只搪瓷杯共用 region/pose 投影和检查点恢复，杯子还验证了任意 37° 局部朝向；
...
- 建造退出与物品空间 Phase A/B：区域几何 + 存档契约定向 EditMode 18/18（job `a8cc82597a4b`），扩大至全部 NomadWorkshop EditMode 172/172（job `cc8a6aea8124`），完整 Foundation PlayMode 29/29（job `2be22f6525ed`）；覆盖建造切换到居民/开发/关闭后的完整诊断清理、两种 footprint 同区容量、杯具任意 37° 朝向、场景定义接线、精确检查点恢复和既有水循环/故障回归。Game View 已检查厨房台面杯具及开发面板 owner/region/local pose，本地忽略证据为 `Screenshots/nomad-world-item-cup-close-v1.png`、`Screenshots/nomad-world-item-developer-v1.png`；
...
- 水罐寻找与最小旱厕使用已接入正式 Foundation；一只空杯已验证台面占地、表现和存档，但还不会被居民拿取或用于饮水。厕所清运、其他食物容器 / 餐具、洗手、洁净度和晕车仍只有旧 Harness 或纯 C# 方案规则，尚未接入完整设施、动画或长期平衡；
...
下一步先完成杯具 Phase B 后半：只用当前一只杯子验证来源/目标区域联合预留、居民拿起/携带/放下的原子所有权转移，以及通用手部锚点/最薄 IK，不横向增加整套餐饮细节。随后用多 Seed、不同帧步长、暂停 / 加速和中途存取的长时 Harness 检查故障时间、需求、休闲选择与资源守恒分布；再让真实沙尘天气进入设施暴露入口，并把一键修理升级为“居民前往维修位 → 消耗备件 → 可中断维修 → 原子提交”。一份餐食、蓝图搬料施工、玩家存档 UI 和正式多居民 Agent 继续按风险后移。
