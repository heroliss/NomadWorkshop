...
namespace Game.NomadWorkshop.PlayMode.Tests
{
    ...
    public class NomadStairTraversalPlayModeTests
    {
        ...
        private void AssertPalmContact()
        {
            ...
            Transform palm = _view.Container.RightPalm;
            ...
            {
                foreach (var volume in _view.Container.ClearanceVolumes)
                {
                    Vector3 scale = volume.frame.lossyScale;
                    var bounds = volume.bounds; bounds.Expand(new Vector3(.06f / scale.x, .06f / scale.y, .06f / scale.z));
                    Vector3 a = volume.frame.InverseTransformPoint(from), b = volume.frame.InverseTransformPoint(to);
                    bool intersects = bounds.Contains(a) || (bounds.IntersectRay(new Ray(a, b - a), out float hit) &&
                        hit <= Vector3.Distance(a, b));
